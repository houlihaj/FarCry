...
//			structure that maps virtual key codes to their human-readable names.
struct XT_VKEYMAP
{
...
// Summary: CMap definition for mapping accelerator accelerator swap out item lists 
...
public:
	...
	// Returns: true if the accelerator data has been initialized, otherwise returns false.
	// Summary: This member function will check to see whether or not the accelerator data has been
	//			initialized. 
	bool IsInitialized();
	...
	//			defined accelerator tables for the manager. It must be called after the
	...
	//			(i.e. "Ctrl+C").
	// Summary: This member function will, upon being given a command ID, return a string describing
